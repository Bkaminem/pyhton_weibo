...
csv_path = os.path.join(base_path, 'gjc.csv')
    ...
    def get_yonghu_info(self):
        ...
        try:
            for i in range(1,100):
                url1 = 'https://weibo.cn/search/mblog?hideSearchFrame=&keyword='+name+'&page={}'.format(i+1)
                print(url1)
                ...
                if len(target) > 5:
                    for j in range(4, len(target) - 2):
                        ...
                        sql = 'INSERT INTO gjc (`user_name`, `content`, `post_time`,`zf`, `num_zan`, `num_forw`, `num_comm`, `link`)\
